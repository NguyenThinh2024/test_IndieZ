...
namespace Thinh.Base
{
    public class VibrationController : MonoSingleton<VibrationController>
    {
        ...
        {
            StartCoroutine(PlayInternal(type, delayTime));
        }

        private IEnumerator PlayInternal(HapticType type, float delayTime)
        {
            if (delayTime > 0f)
            {
        ...
        private void PlayHaptic(HapticType type)
        {
            ...
            Handheld.Vibrate();
            Log($"iOS Handheld.Vibrate -> {type}");
#else
            Log($"Native haptic plugin unavailable -> {type}");
#endif
        }
